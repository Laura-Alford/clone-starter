module.exports = function(app, passport, db, multer, ObjectId) {
 ...
 app.post('/makePost', upload.single('file-to-upload'), (req, res) => {
   ...
   db.collection('posts').save({caption: req.body.caption, likes: 0, img: 'images/uploads/' + req.file.filename, postedBy: user}, (err, result) => {
     if (err) return console.log(err)
 ...
 // comment routes ===============================================================
     app.get('/post/comment:id', isLoggedIn, function(req, res) {
       var postId = ObjectId(req.params.id)
       db.collection('posts').find({postedBy: postId}).toArray((err, result) => {
         if (err) return console.log(err)
         res.render('post.ejs', {
